...
The processing and transformation behavior is implemented with the current Flask and Pillow runtime dependencies. SQLite is now used at runtime for persistent image-session metadata, editing history, projects, layers, and image-layer asset metadata; image binaries remain in file storage, including the `layer-assets` category. OpenCV and NumPy remain part of the documented architectural direction but are not current runtime dependencies in `backend/requirements.txt`.
