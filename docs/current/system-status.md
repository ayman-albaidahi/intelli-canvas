...
| Check | Result |
|---|---:|
| Automated Python tests | 259 passed |
| Ruff (`ruff check backend tests`) | Passed |
| Ruff format (`ruff format --check`) | 82 files already formatted |
| `compileall` | Passed |
...
| CI on `main` | `.github/workflows/ci.yml` runs compileall, pytest, ruff, pip-audit (frontend checks pending) |
