...
def _image_id_and_params():
    ...
    if not isinstance(payload, dict):
        return None, None, error_response("INVALID_REQUEST", "A JSON request body is required.", 400)
    image_id = payload.get("image_id")
    if not isinstance(image_id, str) or not image_id.strip():
        return None, None, error_response("INVALID_IMAGE_ID", "A valid image_id is required.", 400)
    if current_app.config["IMAGE_SESSION_SERVICE"].get_session(image_id) is None:
        return None, None, error_response("IMAGE_SESSION_NOT_FOUND", "Image session was not found.", 404)
    try:
        params = BackgroundService.validate_params(payload)
    except BackgroundParamError as exc:
        return None, None, error_response("INVALID_MASK_PARAMS", str(exc), 400)
...
def mask_preview():
    ...
    try:
        ...
        return error_response("MASK_PREVIEW_FAILED", "The mask could not be generated.", 400)
...
def remove_background():
    ...
    try:
        ...
        return error_response("REMOVE_FAILED", "The background could not be removed.", 400)
...
def replace_background():
    ...
    try:
        ...
        return error_response("REPLACE_FAILED", "The background could not be replaced.", 400)
    return jsonify(success=True, image={key: value for key, value in result.items() if key != "path"})
