...
class SQLiteSessionRepository:
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        with self._connect() as connection:
            ids = [row[0] for row in connection.execute("SELECT image_id FROM image_sessions").fetchall()]
    ...
    def update_current_image(self, image_id: str, filename: str, storage: str, operation: str | None, now: int) -> dict[str, Any]:
        with self._connect() as connection:
            ...
            if session is None:
                raise FileNotFoundError("Image session was not found.")
            connection.execute("UPDATE image_sessions SET current_filename = ?, current_storage = ?, updated_at = ? WHERE image_id = ?", (filename, storage, now, image_id))
            if operation:
    ...
    def set_current_history(self, image_id: str, index: int, now: int) -> dict[str, Any]:
        with self._connect() as connection:
            row = connection.execute("SELECT filename, storage FROM image_history WHERE image_id = ? AND history_index = ?", (image_id, index)).fetchone()
            if row is None:
                raise ValueError("History index is out of range.")
            connection.execute("UPDATE image_sessions SET current_filename = ?, current_storage = ?, history_index = ?, updated_at = ? WHERE image_id = ?", (row["filename"], row["storage"], index, now, image_id))
    ...
    def clear_history(self, image_id: str, now: int) -> dict[str, Any]:
        with self._connect() as connection:
            row = connection.execute("SELECT current_filename, current_storage FROM image_sessions WHERE image_id = ?", (image_id,)).fetchone()
            if row is None:
                raise FileNotFoundError("Image session was not found.")
    ...
    def save_layers(self, image_id: str, layers: list[dict[str, Any]], now: int) -> list[dict[str, Any]]:
        with self._connect() as connection:
            if connection.execute("SELECT 1 FROM image_sessions WHERE image_id = ?", (image_id,)).fetchone() is None:
                raise FileNotFoundError("Image session was not found.")
            connection.execute("DELETE FROM image_layers WHERE image_id = ?", (image_id,))
            for index, layer in enumerate(layers):
                connection.execute("INSERT INTO image_layers (image_id, layer_id, z_index, layer_type, payload_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)", (image_id, layer["id"], index, layer["type"], json.dumps(layer, separators=(",", ":")), now, now))
