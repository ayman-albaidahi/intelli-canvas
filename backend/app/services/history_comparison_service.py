...
class HistoryComparisonService:
    ...
    def compare(self, image_id: str, from_index: int, to_index: int) -> dict:
        ...
        with (
            Image.open(self.path_for(image_id, from_index)) as source_image,
            Image.open(self.path_for(image_id, to_index)) as target_image,
        ):
            source_size = source_image.size
            target_size = target_image.size
        return {
            "from": {
                **self._public_entry(source),
                "width": source_size[0],
                "height": source_size[1],
            },
            "to": {
                **self._public_entry(target),
                "width": target_size[0],
    ...
    def diff_bytes(
        ...
        with (
            Image.open(self.path_for(image_id, from_index)) as before,
            Image.open(self.path_for(image_id, to_index)) as after,
        ):
            ...
            if mode == "absolute":
                ...
                if mode == "heatmap":
                    result = ImageOps.colorize(
                        score, black="#101018", white="#ff5b78"
                    ).convert("RGBA")
                else:
                    result = Image.merge(
                        "RGBA", (score, score, score, Image.new("L", score.size, 255))
                    )
                score.close()
            buffer = BytesIO()
