...
def _image(client):
    ...
    source.seek(0)
    response = client.post("/api/images", data={"file": (source, "pipeline.png")}, content_type="multipart/form-data")
...
def test_pipeline_get_put_and_node_crud():
    ...
    added = client.post("/api/pipeline/nodes", json={"image_id": image_id, "operation": "brightness", "parameters": {"value": 120}})
    ...
    patched = client.patch(f"/api/pipeline/nodes/{node_id}", json={"image_id": image_id, "parameters": {"value": 140}})
    assert patched.status_code == 200
    assert patched.get_json()["pipeline"]["nodes"][0]["parameters"] == {"value": 140}

    toggled = client.post(f"/api/pipeline/nodes/{node_id}/toggle", json={"image_id": image_id})
    assert toggled.get_json()["pipeline"]["nodes"][0]["enabled"] is False

    put = client.put("/api/pipeline", json={"image_id": image_id, "version": 9, "nodes": [{"id": "a", "operation": "grayscale", "parameters": {}, "enabled": True}, {"id": "b", "operation": "blur", "parameters": {"value": 3}, "enabled": True}]})
    assert put.status_code == 200
    assert [node["id"] for node in put.get_json()["pipeline"]["nodes"]] == ["a", "b"]

    reordered = client.post("/api/pipeline/reorder", json={"image_id": image_id, "node_id": "b", "order": 0})
    assert [node["id"] for node in reordered.get_json()["pipeline"]["nodes"]] == ["b", "a"]
...
def test_pipeline_rejects_unknown_operations_duplicate_ids_and_bad_enabled():
    ...
    bad_enabled = client.post("/api/pipeline/nodes", json={"image_id": image_id, "operation": "grayscale", "enabled": "yes"})
...
def test_pipeline_is_session_scoped():
    ...
    created = client.post("/api/pipeline/nodes", json={"image_id": image_id, "operation": "negative"})
    assert created.status_code == 200
    assert client.get(f"/api/pipeline?image_id={image_id}").get_json()["pipeline"]["nodes"][0]["operation"] == "negative"
